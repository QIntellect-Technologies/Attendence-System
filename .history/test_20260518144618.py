...
def test_face_quality():
    ...
    try:
        ...
        # Create test frames
        # Good quality frame
        good_frame = np.random.randint(80, 180, (480, 640, 3), dtype=np.uint8)
        ...
        quality_good = fp.assess_face_quality(good_frame, (100, 100, 200, 200))
        quality_blurry = fp.assess_face_quality(blurry_frame, (100, 100, 200, 200))
        quality_dark = fp.assess_face_quality(dark_frame, (100, 100, 200, 200))
        
        logger.info(f"✓ Good frame quality: {quality_good['score']:.3f}")
        logger.info(f"✓ Blurry frame quality: {quality_blurry['score']:.3f}")
        logger.info(f"✓ Dark frame quality: {quality_dark['score']:.3f}")
        
        if quality_good['score'] > quality_blurry['score']:
            logger.info("✓ Quality assessment working correctly")
            return True
        else:
            logger.warning("⚠ Quality assessment might need tuning")
